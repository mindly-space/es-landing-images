...
export const ContentSection: React.FC = () => {
  ...
  const headingSpanish = "¿Necesitas ayuda para elegir psicólog@? Escríbenos y te ayudaremos a encontrar al ";
  const highlightedEs = "terapeuta ideal para ti";
  const headingEnglish = "Still need help choosing the right therapist? Feel free to chat — we'll help you";
  const highlightedEng = "find the perfect match!";
  
  const subtitleSpanish = "Nuestro equipo te ayudará a encontrar al especialista que mejor se adapte a tus necesidades";
  const subtitleEnglish = "Our team is here to connect you with a professional who meets your personal needs and preferences";
  ...
  return <div className="w-full max-w-full mx-auto px-4 md:px-16 flex flex-col items-center pt-2 mt-20px md:mt-5 relative z-10">
      <motion.div 
        ...
        <div className="w-full">
          <h2 className="text-[18px] md:text-[36px] leading-[120%] font-dela text-[#212121] leading-tight w-full max-w-[90%] mx-auto">
            {isEnglish 
              ? <>
                  {headingEnglish}{" "}
                  <span className="text-[rgba(2,178,134,1)]">{highlightedEng}</span>
                </>
              : <>
                  {headingSpanish}
                  <span className="text-[rgba(2,178,134,1)]">{highlightedEs}</span>
                </>
            }
          </h2>
      ...
      >
        ...
        >
          <img src="/lovable-uploads/97cf60bd-9979-4a33-829f-5591e6c31641.png" alt="WhatsApp" className="w-[20px] h-[20px] mr-2 object-cover" />
